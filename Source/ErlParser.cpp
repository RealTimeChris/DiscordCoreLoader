...
namespace DiscordCoreLoader {
	...
	void EtfSerializer::appendUint64(uint64_t valueNew) {
	...
	void EtfSerializer::appendInt64(int64_t valueNew) {
		char newBuffer[11]{ static_cast<int8_t>(EtfType::Small_Big_Ext) };
		char encodedBytes{};
		while (valueNew > 0) {
			newBuffer[3 + encodedBytes] = valueNew & 0xFF;
	...
	void EtfSerializer::appendUint32(const uint32_t valueNew) {
		char newBuffer[5]{ static_cast<uint8_t>(EtfType::Integer_Ext) };
		storeBits(newBuffer + 1, valueNew);
		writeString(newBuffer, std::size(newBuffer));
	}

	void EtfSerializer::appendInt32(const int32_t valueNew) {
		char newBuffer[5]{ static_cast<uint8_t>(EtfType::Integer_Ext) };
		storeBits(newBuffer + 1, valueNew);
		writeString(newBuffer, std::size(newBuffer));
	}

	void EtfSerializer::appendUint8(const uint8_t valueNew) {
		char newBuffer[2]{ static_cast<uint8_t>(EtfType::Small_Integer_Ext), static_cast<char>(valueNew) };
		writeString(newBuffer, std::size(newBuffer));
	}

	void EtfSerializer::appendInt8(const int8_t valueNew) {
		char newBuffer[2]{ static_cast<uint8_t>(EtfType::Small_Integer_Ext), static_cast<char>(valueNew) };
		writeString(newBuffer, std::size(newBuffer));
	}
