...
namespace DiscordCoreLoader {
	...
	std::unique_ptr<GuildMemberData> ObjectGenerator::generateGuildMember(const std::string& guildId, const std::vector<std::unique_ptr<RoleData>>& roles) {
	...
	std::unique_ptr<GuildData> ObjectGenerator::generateGuild(std::string guildId) {
		...
		for (uint64_t x = 0; x < theData->memberCount; x += 1) {
			theData->members.push_back(this->generateGuildMember(guildId, theData->roles));
		}
