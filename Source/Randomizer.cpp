	...
	void Randomizer::randomizeId(std::string& theString, uint64_t minValue, uint64_t maxValue) {
		...
		if (returnString.size() > 18) {
			...
			for (int32_t x = 0; x < theString.size();++x) {
				if (static_cast<char>(theString[x]) == static_cast<char>(',') || static_cast<char>(theString[x]) == '\'' || static_cast<char>(theString[x]) == '/' ||
					static_cast<char>(theString[x]) == '\"' || static_cast<char>(theString[x]) == '\\' || static_cast<char>(theString[x]) == '`' || static_cast<char>(theString[x]) == '{' ||
					static_cast<char>(theString[x]) == '}' || static_cast<char>(theString[x]) == '[' || static_cast<char>(theString[x]) == ']' || static_cast<char>(theString[x]) == '(' ||
					static_cast<char>(theString[x]) == ')' || static_cast<char>(theString[x]) == static_cast<char>(0) || static_cast<char>(theString[x]) == static_cast<char>(1)) {
