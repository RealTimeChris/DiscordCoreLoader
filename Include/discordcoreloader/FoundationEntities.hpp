...
namespace DiscordCoreLoader {
	...
	struct RequestGuildMembersData {
		...
		std::string query{};///< String string that username starts with, or an empty string to return all members. one of query or user_ids.
	...
	template<typename TimeType> class StopWatch {
	  public:
		...
		StopWatch(TimeType maxNumberOfMsNew) {
			...
			this->startTime = static_cast<uint64_t>(std::chrono::duration_cast<TimeType>(std::chrono::system_clock::now().time_since_epoch()).count());
		}
		...
		uint64_t totalTimePassed() {
			...
			uint64_t elapsedTime = currentTime - this->startTime;
			return elapsedTime;
		}
